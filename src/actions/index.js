...
const updateListTitleAction = list => ({
...
export const addNewList = listname => (dispatch, getState) => {
  const payload = { title: listname, cards: [] };
  dispatch(addNewListAction(payload));
  const state = getState().List.List.List[getState().List.List.List.length - 1]
    .id;
  return Promise.resolve(state);
};

export const updateListTitle = (listId, boardId, title) => dispatch => {
  dispatch(updateListTitleAction({ listId, boardId, title }));
...
export const openQuickEditor = (
  ...
  top = Math.ceil(top);
  left = Math.ceil(left);
  const payload = { listId, cardId, cardTitle, top, left, height };
